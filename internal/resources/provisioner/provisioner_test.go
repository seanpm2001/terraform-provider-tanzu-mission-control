...
func TestAcceptanceForProvisionerResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: getTestProvisionerWithResourceConfigValue(provisionerName, "eks"),
				Check: resource.ComposeTestCheckFunc(
					checkResourceAttributes(provider, provisionerResourceName, provisionerName),
					//verifyProvisionerResourceCreation(provider, provisionerResourceName, provisionerName),
					//resource.TestCheckResourceAttr(provisionerResourceName, "name", provisionerName),
				),
			},
		},
	})
...
func checkResourceAttributes(provider *schema.Provider, resourceName, prvName string) resource.TestCheckFunc {
	var check = []resource.TestCheckFunc{
		...
		resource.TestCheckResourceAttr(resourceName, "name", prvName),
	}
...
func getTestProvisionerWithResourceConfigValue(prvName, mgmtClusterName string) string {
	return fmt.Sprintf(`
	resource "%s" "%s" {
		name = "%s"
	...
	`, resourceName, resourceVar, prvName, mgmtClusterName, testhelper.MetaTemplate)
}
